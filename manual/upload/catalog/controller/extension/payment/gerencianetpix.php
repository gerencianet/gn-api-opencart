...
class ControllerExtensionPaymentGerencianetPix extends Controller {
    ...
    public function index() {
        ...
        if($this->session->data['currency'] != Constants::constantValues['currency']) {
            $messageError = $this->language->get('message_error_gerencianetpix');
			
            return "<span><h4>$messageError</h4></span>";
        }

        $data['action'] = $this->url->link('extension/payment/gerencianetpix/finalize');
        $data['button_confirm'] = $this->language->get('button_confirm');

        // Valor do Desconto em %
        $data['discount'] = $this->config->get('payment_gerencianetpix_discount');     
        ...
        return $this->load->view('extension/payment/gerencianetpix', $data);
    }
    ...
    private function createCharge($orderTotal, $order_id) {
        ...
        $dataDB = $this->model_extension_payment_gerencianetpix->find('order_id', $order_id);
        ...
        if(isset($dataDB['tx_id'])) {
            ...
            if($apiResponse['txid']) {
                ...
                $this->model_extension_payment_gerencianetpix->insert($data);
    ...
    */
    ...
    public function callback() {
        ...
        if (empty($pixPaymentData)) {
            ...
            $this->model_extension_payment_gerencianetpix->update($conditions, $data);

            // Busca no banco o id do Pedido
            $column = 'tx_id';
            $dataDB = $this->model_extension_payment_gerencianetpix->find($column, $txID);
