	...
	public function pay_billet() {
		...
		if ($order_info) {
			...
			try {
			    ...
			    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_gerencianet_waiting_status_id'), '<a href="' . $charge['data']['pdf']['charge'] . '" target="_blank">' . $this->language->get('gn_billet_oc_order_comment') . '</a>', true);
