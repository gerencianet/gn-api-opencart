...
class ControllerPaymentGerencianet extends Controller {
	public function index() {
		...
		if ($order_info) {
			...
			if ($gn_checkout_type=="1") {
				...
				$data['order_total_billet_formatted'] = $this->currency->format($order_info['total']-(intval((($this->getBilletDiscount())*($order_info['total']-$shipping_price)))/100));
				$data['order_total_card_formatted'] = $this->currency->format($order_info['total']);
				...
				$data['discount_total_value'] = $this->currency->format(intval((($this->getBilletDiscount())*($order_info['total']-$shipping_price)))/100);
				$data['gn_warning_sandbox_message'] = "MENSAGEM SANDBOX ATIVO";
		...
		if ($order_info) {
			...
			foreach ($order_data['totals'] as $total) {
				if (intval(floatval($total['value'])*100)==intval(floatval($order_info['total'])*100)) {
					$data['totals_billet'][] = array(
						'title' => $total['title'],
						'text'  => $this->currency->format($total['value']-(($this->getBilletDiscount()/100)*$this->cart->getSubTotal())),
					);
				} else {
					$data['totals_billet'][] = array(
						'title' => $total['title'],
				...
				if (!$billetDiscountAdded && $this->getBilletDiscount()>0) {
					$data['totals_billet'][] = array(
						...
						'text'  => '-' . $this->currency->format(intval(($this->getBilletDiscount())*$this->cart->getSubTotal())/100),
					);
			...
			$data['total_paying_with_discount'] = $this->currency->format($total['value']-intval(($this->getBilletDiscount())*$this->cart->getSubTotal())/100);
	...
	public function payment_billet_option() {
		$this->load->language('payment/gerencianet');
		$this->load->model('checkout/order');
		...
		if ($order_info) {
			...
			if (isset($order_info['phone_number'])) {
				$data['phone_number'] = html_entity_decode($order_info['phone_number'], ENT_QUOTES, 'UTF-8');
			} else {
				$data['phone_number'] = '';
			}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gerencianet_payment_billet.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/gerencianet_payment_billet.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/payment/gerencianet_payment_billet.tpl', $data));
			}
		}
		
	}

	public function payment_card_option() {
		$this->load->language('payment/gerencianet');
		$this->load->model('checkout/order');
		...
		if ($order_info) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gerencianet_payment_card.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/gerencianet_payment_card.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/payment/gerencianet_payment_card.tpl', $data));
			}
		}
	}
	...
	public function pay_billet() {
		...
		if ($order_info) {
			...
			foreach ($order_data['totals'] as $total) {
				...
				if (floatval($total['value'])<0) {
					$total_discount += $this->formatMoney($this->currency->format($total['value']), true);
				}
			}

			$discountFormatedByOC = $this->formatMoney($this->currency->format(($this->getBilletDiscount()/100)*$this->cart->getSubTotal()), true);
			$discountFormatedByOC_no_currency_format = (intval(($this->getBilletDiscount())*$this->cart->getSubTotal())/100);
			...
			try {
			    ...
			    $totalWithBilletDiscount = ($order_info['total']-floatval($total_billet_discount_no_currency_format));
	...
	public function pay_card() {
		...
		if ($order_info) {
			...
			foreach ($order_data['totals'] as $total) {
				...
				if (floatval($total['value'])<0) {
					$total_discount += $this->formatMoney($total['value'], true);
				}
			}
